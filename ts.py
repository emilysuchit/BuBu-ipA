...
PARALLEL_TIMEOUT = int(os.environ.get("PARALLEL_TIMEOUT", 60))
...
def pick_addr(url, cc=None, rc=None):
    ...
    # 1. Exact country code match in domain suffix
    ...
    # 2. Special TLD overrides (e.g. .uk → GB)
...
def is_captcha_required(response_text):
    ...
    # ✅ IMPROVED: Try JSON parse first for accurate detection
    try:
...
async def make_graphql_request_with_captcha_handling(
    ...
    # ✅ FIXED: removed unused solve_captcha param, removed unreachable return
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        address_info = pick_addr(ourl, cc=None, rc=None)
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            # ── Extract build ID, source token, identification signature ──
            ...
            # ✅ FIXED: use unescaped_text for source_token extraction
            ...
            if payment_data and payment_data.get("__typename") == "FilledPaymentTerms":
                ...
                for method in payment_methods:
                    ...
                    if payment_method.get("name") or payment_method.get("paymentMethodIdentifier"):
                        ...
                        displayName = payment_method.get("extensibilityDisplayName") or payment_method.get(
                            "name", "Unknown"
                        )
                        gateway = payment_method.get("extensibilityDisplayName") or payment_method.get(
                            "name", "UNKNOWN"
                        )
            ...
            vault_headers.update({
                "shopify-checkout-source": f'id="{attempt_token}", type="cn"',
                "x-checkout-one-session-token": sst,
            })

            # ✅ FIXED: url → ourl
            vault_url = f"https://{urlparse(ourl).netloc}/checkouts/unstable/{attempt_token}/sessions/{sst}/credit_cards.json"
            ...
            # ✅ IMPROVED: parse JSON first, then check error messages
            try:
                ...
                try:
                    ...
                    if receipt_data:
                        ...
                        if typename == "ProcessedReceipt":
                            ...
                            # ✅ FIXED: non-PaymentFailed errors → return False (not True)
...
def shopify_checker_parallel():
    ...
    try:
        ...
        # ✅ FIXED: proper race-condition-free slot checking
        while True:
            with _request_lock:
